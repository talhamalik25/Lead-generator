...
function LeadForm({ initial, onSave, onCancel }) {
  const [form, setForm] = useState(() => normalizeLead(initial ?? {}))
...
function resultKey(result) {
  return `${(result.name ?? '').toLowerCase()}|${(result.phone ?? '').toLowerCase()}`
}

function isLeadDuplicate(leads, result) {
  const name = (result.name ?? '').toLowerCase()
  return leads.some((l) => (l.name ?? '').toLowerCase() === name)
}

function FindLeadsPanel({ leads, onAddLead }) {
  ...
  const handleSearch = async (e) => {
    ...
    if (!trimmedNiche || !trimmedArea) return
    ...
    try {
      const res = await fetch('/api/find-leads', {
        ...
        body: JSON.stringify({ niche: trimmedNiche, area: trimmedArea }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Search failed')

      setResults(data.results ?? [])
      setSearchedNiche(trimmedNiche)
      if (!data.results?.length) {
        setError('No businesses found. Try a different niche or area.')
      }
    } catch (err) {
  ...
  const handleAdd = (result) => {
    onAddLead(result, searchedNiche)
    setAddedKeys((prev) => new Set(prev).add(resultKey(result)))
  }

  return (
    <div className="find-leads">
      <div className="find-leads-intro">
        <Sparkles size={20} color="#00E6D9" />
        <p>Search OpenStreetMap&apos;s public business data for local businesses that may need a website. Results are free but sometimes incomplete — always verify before outreach.</p>
      </div>

      <form className="find-leads-form" onSubmit={handleSearch}>
        <div className="form-group">
          <label htmlFor="find-niche">Niche</label>
          <input
            id="find-niche"
            value={niche}
            onChange={(e) => setNiche(e.target.value)}
            placeholder="e.g. cafes, salons, clinics"
            required
          />
        </div>
        <div className="form-group">
          <label htmlFor="find-area">Area</label>
          <input
            id="find-area"
            value={area}
            onChange={(e) => setArea(e.target.value)}
            placeholder="e.g. DHA Karachi, Gulshan-e-Iqbal"
            required
          />
        </div>
        <button type="submit" className="btn-primary" disabled={loading}>
          {loading ? (
            <><Loader2 size={18} className="spin" style={{ verticalAlign: 'middle', marginRight: 6 }} />Searching...</>
          ) : (
            <><Search size={18} style={{ verticalAlign: 'middle', marginRight: 6 }} />Find leads</>
          )}
        </button>
      </form>
      ...
      <div className="disclaimer">
        <AlertCircle size={16} />
        <span>Results come from OpenStreetMap — free, but sometimes incomplete or outdated. Verify each business before reaching out.</span>
      </div>
      ...
      {results.length > 0 && (
        <div className="find-results">
          <p className="find-results-heading">{results.length} potential lead{results.length !== 1 ? 's' : ''} found</p>
          {results.map((result, i) => {
            const key = resultKey(result)
            const alreadyAdded = addedKeys.has(key) || isLeadDuplicate(leads, result)
            return (
              <div key={`${key}-${i}`} className="find-result-card">
                <p className="find-result-name">{result.name}</p>
                {result.area && (
                  <p className="find-result-row">
                    <MapPin size={14} />
                    {result.area}
                  </p>
                )}
                ...
                {result.reason && (
                  <p className="find-result-reason">{result.reason}</p>
                )}
                <button
                  type="button"
                  className={`btn-add-tracker${alreadyAdded ? ' added' : ''}`}
                  onClick={() => handleAdd(result)}
                  disabled={alreadyAdded}
                >
                  {alreadyAdded ? (
                    <><Check size={16} />Added to tracker</>
                  ) : (
                    <><UserPlus size={16} />Add to tracker</>
                  )}
                </button>
              </div>
            )
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setLeads(rawLeads ? JSON.parse(rawLeads).map(normalizeLead) : [])
      const rawName = localStorage.getItem(USER_NAME_KEY) || ''
      setUserName(rawName)
      setSettingsName(rawName)
    } catch {
      // fallback
    }
  }, [])
  ...
  const addLeadFromSearch = (result, niche) => {
    const contactParts = [result.phone, result.area].filter(Boolean)
    const notes = [result.reason, result.area ? `Area: ${result.area}` : ''].filter(Boolean).join('\n')
    const lead = {
      ...normalizeLead({
        name: result.name,
        platform: 'Other',
  ...
  if (!mounted) {
    // Avoid dynamic server components rendering mismatch on loading screen
    return null
  }
  ...
  return (
    <div className="app">
      ...
      {activeTab === 'tracker' ? (
        ...
        <FindLeadsPanel leads={leads} onAddLead={addLeadFromSearch} />
      )}

      {activeTab === 'tracker' && !sheet && (
        <button
          type="button"
          className="fab"
